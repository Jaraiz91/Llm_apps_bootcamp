...
llama_llm = ChatGroq(model='groq/llama-3.3-70b-versatile')
llama_llm = OllamaLLM(model='llama3')
gpt_llm = ChatOpenAI(model='openai/gpt-3.5-turbo-0125')

class ProcessSearchTool(BaseTool):
    name: str = 'Process Search Tool'
    description: str = 'Tool used to process content found on the internet'
...
class TavilySearchTool(BaseTool):
    ...
    description: str = 'Tool for searching info on the internet'
    def _run(self):
        tavily_searcher = TavilySearchResults(max_results=1)
        return tavily_searcher
    
process_tool = ProcessSearchTool()
tavily_search = TavilySearchTool()
scrapper = ScrapeWebsiteTool()
tools = [tavily_search,process_tool, scrapper]
...
start_crew(crew=crew)
print('--------------------------------')
print('Subject Report:', task1.output)
print('--------------------------------')
print('Article:', task2.output)
print('--------------------------------')
print('tweet:', task3.output)
print('--------------------------------')
print('final report', task4.output)